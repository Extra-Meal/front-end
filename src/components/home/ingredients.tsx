            ...
            <div className="grid grid-cols-1 gap-7 md:grid-cols-2 lg:grid-cols-3">
                {
                    ingredients.map((ingredient, index) => (
                        <div className="product-item px-[35px] relative z-10 text-center mt-[120px] mb-[80px]">
                            <div className="product-img flex justify-center items-end -mb-[120px] transform -translate-y-[140px]">
                                <Link to={`/ingredients/${ingredient.strIngredient}`}>
                                    <img src={`https://www.themealdb.com/images/ingredients/${ingredient.strIngredient}-Small.png`} alt="" className="object-cover transition-all transform translate-y-0 w-70 h-70" />
                                </Link>
                            </div>
                            <div className="product-content px-[20px] pb-[40px]">
                                <h4 className="title font-bold text-3xl mb-[20px] transform uppercase">
                                    <Link to={`/ingredients/${ingredient.strIngredient}`}>{ingredient.strIngredient}</Link>
                                </h4>
                                <h6 className="price text-primary text-2xl mb-2">$4.99</h6>
                                <p className="desc text-base font-light text-muted-foreground">{ingredient.strDescription.split(".")[0]}.</p>
                            </div>
                            <div className="product-shape absolute top-0 left-0 w-full h-full -z-1">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 401 314" preserveAspectRatio="none" fill="rgba(175, 62, 62, 0.1)" className="block absolute w-full h-full overflow-visible">
